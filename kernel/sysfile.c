...
sys_munmap(void)
{
  ...
  for (int i = 0; i < NVMAS; ++i) {
    ...
    if (vma->used && vma->address <= addr && addr < vma->address + vma->length) {
      ...
      if (vma->address == addr) {
        if (PGROUNDDOWN(addr) != PGROUNDDOWN(vma->address))
          uvmunmap(p->pagetable, PGROUNDDOWN(vma->address),
                   (PGROUNDDOWN(addr) - PGROUNDDOWN(vma->address)) / PGSIZE, 1);
        vma->address += length;
        vma->length -= length;
        vma->offset += length;
        return 0;
      }
      if (vma->address + vma->length == addr + length) {
