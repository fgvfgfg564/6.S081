...
userinit(void)
{
  ...
  // and data into it.
  uvminit(p->pagetable, initcode, sizeof(initcode), p->kernel_pagetable);
  p->sz = PGSIZE;
...
growproc(int n)
{
  ...
  sz = p->sz;
  if(n > 0){
    if((sz = uvmalloc(p->pagetable, sz, sz + n, p->kernel_pagetable)) == 0) {
      return -1;
    }
  } else if(n < 0){
    sz = uvmdealloc(p->pagetable, sz, sz + n, p->kernel_pagetable);
  }
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable, np->pagetable, p->sz, np->kernel_pagetable) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
